...
const ManageOptions: NextPage<Props> = ({ visible, onClose }) => {
	...
	const handleAddOption = () => {
		if (newOption.title && newOption.url) {
			const optionExists = options.some(
				(option) => option.title === newOption.title
			)
	...
	return (
		<div className='fixed inset-0 bg-black bg-opacity-10 backdrop-blur-sm flex items-center justify-center mx-3 z-20'>
			<div className='bg-black border border-slate-400 w-96 p-6 rounded-lg'>
				<div className='flex justify-between flex-row mb-4'>
					<h2 className='text-md font-medium'>Mina drycker</h2>
					<button
						type='button'
						className='self-end text-white p-2 bg-slate-500 rounded hover:text-gray-800'
						onClick={onClose}
					>
						<svg
				...
				{options.length > 0 ? (
					...
					<p className='text-center font-regular text-sm text-slate-400 mb-6'>
						Inga tillagda drycker
					</p>
				)}
				...
				<div>
					...
					<div className='w-100'>
						<button
							type='button'
							className='mt-4 w-full px-4 py-2 rounded-md btn-search'

...
const AreaInput: NextPage<Props> = ({ onInputChange }) => {
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredSuggestions = cities
      .filter((city) => city.toLowerCase().includes(value.toLowerCase().trim()))
      .filter(
        (suggestion) => suggestion.toLowerCase() !== value.toLowerCase().trim()
      );
          ...
          {savedAreas.length > 0 && (
            <div>
              <h4 className="text-center text-xs text-slate-500 my-2">
                Sparade Städer<i className="ms-2 fa-solid fa-location-dot"></i>
              </h4>
              <ul className="p-2">
